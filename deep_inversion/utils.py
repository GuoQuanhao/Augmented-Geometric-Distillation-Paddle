...
def clip(image_numpy, use_fp16=False):
    """
    ...
    for c in range(3):
        m, s = mean[c], std[c]
        image_numpy[:, c] = np.clip(image_numpy[:, c], -m / s, (1 - m) / s)
    return image_numpy


def denormalize(image_numpy, use_fp16=False):
    """
    ...
    for c in range(3):
        m, s = mean[c], std[c]
        image_numpy[:, c] = np.clip(image_numpy[:, c] * s + m, 0, 1)
...
def batch_images_as_tensors(paths):
    ...
    batched = paddle.concat([image.unsqueeze(dim=0) for image in images], dim=0)
    return batched
...
class InversionMetricSampler(object):
    ...
    def __next__(self):
        ...
        indices = [paddle.to_tensor(random.sample(self.ids2index[batch_id], self.samples_per_id), dtype=paddle.int64)
                   for batch_id in batch_ids]
        indices = paddle.concat(indices, dim=0)
...
class EmbeddingsContainer(object):
    def __init__(self, dataloader, model):
        ...
        self.container = {pid: paddle.concat(embeddings, dim=0) for pid, embeddings in self.container.items()}

    def update(self, embeddings, pids):
        print(f"Updating {embeddings.size(0)} embeddings ...")
        for embedding, pid in zip(embeddings, pids):
            self.container[int(pid)] = paddle.concat([self.container[int(pid)], embedding.unsqueeze(dim=0).detach()], dim=0)
